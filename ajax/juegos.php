<div class="row">

    <div class="col-md-6 mt-4">
        <div class="card">
            <div class="card-body" style="text-align: center;">

                <h4> Buscaminas </h4>
                <img src="img/bomb.png" width="100" height="100">
                <br>
                <button id="jugarBuscaminas" class="btn btn-primary col-md-6 mt-3" >
                    <i class="fas fa-play-circle"></i> Jugar
                </button>

            </div>
        </div>
    </div>

    <div class="col-md-6">
        <div class="card mt-4">
            <div class="card-body" style="text-align: center;">

                <h4> Conecta 4 </h4>
                <img src="img/connect4.png" width="100" height="100" style="border-radius: 80px;">
                
                <br>
                <button id="jugarConecta" class="btn btn-primary col-md-6 mt-3" >
                    <i class="fas fa-play-circle"></i> Jugar
                </button>

            </div>
        </div>
    </div>

    <div class="col-md-6">
        <div class="card mt-4">
            <div class="card-body" style="text-align: center;">

                <h4> Tic Tac Toe </h4> 
                <img src="img/tictactoe.png" width="100" height="100" style="border-radius: 80px;">
                
                <br>
                <button id="jugarGato" class="btn btn-primary col-md-6 mt-3" >
                    <i class="fas fa-play-circle"></i> Jugar
                </button>


            </div>
        </div>
    </div>

    <div class="col-md-6">
        <div class="card mt-4">
            <div class="card-body" style="text-align: center;">

                <h4> Ahorcado </h4>
                <img src="img/rope.png" width="100" height="100">
                
                <br>
                <button id="jugarAhorcado" class="btn btn-primary col-md-6 mt-3" >
                    <i class="fas fa-play-circle"></i> Jugar
                </button>

            </div>
        </div>
    </div>

    <script src="js/cargarJuegos.js" charset="utf-8"> </script>

<div>